 ...
 * TODO comments
 */
class GeocoderService_Geonames implements GeocoderService
{
    public function servicepollaction ($searchstring, $searchstringShort, $uri)
    {
        ...
        $url .= urlencode($searchstringShort);
        ...
        if (!isset($result['geonames'][$resultcounter])) {
            return false;
        }
                    ...
                    foreach ($altname as $key => $value) {
                        if ($key == "lang" && $value == "link") {
                            ...
                            if (strpos($linkname, "wikipedia")) {
                                ...
                                $data[$resultcounter]['semantic_data'] = array(
                                    "owl:sameAs" => "http://dbpedia.org/resource" . substr($linkname, $pos)
                                );
                            }
                        }
                        if ($key == "name") {
