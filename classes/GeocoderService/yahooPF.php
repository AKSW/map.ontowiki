 ...
 * TODO comments
 */
class GeocoderService_YahooPF implements GeocoderService
{
    public function servicepollaction ($searchstring, $searchstringShort, $uri)
    {
        $yahooAppid = 'YD-9G7bey8_JXxQP6rxl.fBFGgCdNjoDMACQA--'; // <= Your Yahoo AppID here
        ...
        $url  = "http://where.yahooapis.com/geocode?appid=" . $yahooAppid . "&flags=P&location=";
        ...
        while (isset($result['ResultSet']['Result'][$resultcounter]) && $resultcounter < 10 ) {
            $data[$resultcounter]['name'] = $result['ResultSet']['Result'][$resultcounter]['city'] . ", "
                . $result['ResultSet']['Result'][$resultcounter]['state'] . ", "
                . $result['ResultSet']['Result'][$resultcounter]['country'];
